...
plt.plot(sim.history['z'], sim.history['igm_Tk'],
         label=r'igm_Tk, $V_{\chi b,0}=0$', linestyle='--', c='b')
if sim.pf["dark_matter_heating"]:
    plt.plot(sim.history['z'], sim.history['igm_Tchi'],
             label=r'igm_Tchi, $V_{\chi b,0}=0$', c='b', linestyle='-.')


sim = ares.simulations.Global21cm(
    verbose=False, dark_matter_heating=True, include_cgm=False, initial_redshift=1010)#300)#
sim.run()
# plt.plot(sim.history['z'], 2.73*(1+sim.history['z']),
#          label='Tgamma', c='green', linestyle=':')
plt.plot(sim.history['z'], sim.history['igm_Tk'],
         label='igm_Tk, $V_{\chi b,0}=29$ km/s', linestyle='--', c='r')
if sim.pf["dark_matter_heating"]:
    plt.plot(sim.history['z'], sim.history['igm_Tchi'],
             label='igm_Tchi, $V_{\chi b,0}=29$ km/s', c='r', linestyle='-.')
