...
class MultiPhaseMedium(object):
    ...
    def _initialize_zones(self):
        ...
        for zone in ['igm', 'cgm']:
            ...
            for key in _mpm_defs:# we can see here cgm_ and igm_ isothermal have different results. Bin Xia
            ...
            if zone == 'igm':
                ...
                self.gen_igm = parcel_igm.step()# here I found isothermal = 1, Bin Xia, important point
            ...
            if self.pf['include_cgm']:
                ...
                if not done:
                    ...
                    RC_cgm = self.field.update_rate_coefficients(z,
                        zone='cgm', return_rc=True, cgm_h_1=data_cgm['h_1'])# here it is !!!!!!! zone = cgm, so isothermal change to True. Bin Xia
                    ...
                    t2, dt2, data_cgm = next(self.gen_cgm)# here is a long step. Bin Xia
            else:
                dt2 = 1e50
